...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements PictureService{
    ...
    public PictureVO uploadPicture(MultipartFile multipartFile, PictureUploadRequest pictureUploadRequest, User loginUser) {
        //1.校验参数(如果没登录都能上传文件，安全性就很低了)
        ThrowUtils.throwIf(loginUser == null, ErrorCode.NO_AUTH_ERROR);
        //2.判断是新增还是删除，从 PictureUploadRequest 中看是否能获取得到图片id,也就判断是新增还是删除了
        Long pictureId = null;
        if(pictureUploadRequest != null){
            pictureId = pictureUploadRequest.getId();
        }
        //如果是更新，判断图片是否存在,查询图片在数据库中是否存在
        if(pictureId != null){
            boolean exists = lambdaQuery().eq(Picture::getId, pictureId).exists();
            //如果图片不存在还更新，就是错误的
            ThrowUtils.throwIf(!exists, ErrorCode.NOT_FOUND_ERROR,"图片不存在");
        }
        //如果图片存在，无论是更新还是第一次上传，都是要上传图片
        //3.上传图片，得到图片信息
        //  前缀，我们可以按照用户区划分目录，由于我们这个项目是公共图库
        //  之后我们去开发私有空间，什么企业空间，专属空间，所以我们现在给前缀进行划分
        //  把所有公开的图片上传到 public 目录下，之后如果有私有就上传到 private 目录下，按照用户 id 划分目录
