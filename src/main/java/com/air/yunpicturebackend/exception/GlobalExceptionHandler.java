...
public class GlobalExceptionHandler {
     ...
     * 当 Sa-Token 抛出 NotLoginException 或 NotPermissionException 时
     * 如果没有你的全局异常处理（@ExceptionHandler），Sa-Token 默认会返回 固定格式的 JSON 响应
     * 但通过你的异常拦截后，前端会收到 你自定义的统一响应格式（如 BaseResponse）
     */
    // 捕获 sa-token 的未登录异常，把它改成我们自己的未登录异常，这样我们就统一了异常，前端不会看到 sa-token 抛出我们未处理过的返回结果
    ...
    // 捕获 sa-token 的无权限异常，把它改成我们自己的无权限异常，这样我们就统一了异常，前端不会看到 sa-token 抛出我们未处理过的返回结果
    ...
    public BaseResponse<?> businessExceptionHandler(BusinessException e) {
        ...
        // 给前端返回一个更友好的封装之后的错误信息，封装统一的返回结果
    }
