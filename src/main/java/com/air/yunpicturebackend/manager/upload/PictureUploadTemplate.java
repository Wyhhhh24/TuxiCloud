...
public abstract class PictureUploadTemplate {
    ...
    public UploadPictureResult uploadPicture(Object inputSource, String uploadPathPrefix) {
        ...
        String originFilename = getOriginFilename(inputSource); // 获取原文件名，含文件后缀
        ...
        // 阿里云 AI 扩图之后，将扩图之后的图片调这个接口上传的时候，其 URL 中是有 ？ 和一些 URL 参数
        // 将 ？ 号以及后面的 URL 参数去除掉，得到不会报错的文件名，否则存 COS 的时候会报错，其实上面的 getOriginFilename 里面已经处理过了，这里再来一遍
        ...
        try {
            ...
            // 4.上传图片到对象存储，返回结果中包含图片的基本信息，以及处理之后的图片（压缩图，缩略图）
            ...
            if(CollUtil.isNotEmpty(objectList)){
                // 如果处理后的图片列表为空，也就表示处理失败了，或者压根没有写处理规则
                // 获取压缩之后，得到的文件信息
                CIObject compressedCiObject = objectList.get(0);
                // 缩略图先设置为压缩图，如果缩略图处理成功，那就设置为缩略图
                ...
                //todo 后端数据库中保存的图片url是压缩后的 webp 文件，原图的 url 还没有保存，压缩后的图片基本上一点不模糊，扩
                // 展：1.增加对原图的处理，目前每次上传图片؜实际上会保存原图和压缩图 2 张图片，原图占用的空间还是比较‌大的。如果想进一步优化，可以删除原图，只保留缩略图
                // ；或者在数‍据库中保存原图的地址，用作备份。2）尝试更大比例的压缩，比如使用 质量变换 来处理图片。
            }

            // 6.如果处理失败，那 Picture 记录就封装原图的信息
